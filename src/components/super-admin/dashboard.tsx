...
export function SuperAdminDashboard({ subscribers }: { subscribers: any[] }) {
  ...
  const handleGenerateCode = async (days: number) => {
    ...
    if (res.success) setNewCode(res.message)
    setIsPending(null)
  }
    ...
    <div className="p-6 space-y-8">
      ...
      <div className="bg-white p-6 rounded-lg shadow border border-gray-200 max-w-md">
        ...
        <div className="flex gap-2">
          <button onClick={() => handleGenerateCode(30)} disabled={isPending === "code"} className="bg-blue-600 text-white px-4 py-2 rounded text-sm">
            30 Days
          </button>
          <button onClick={() => handleGenerateCode(365)} disabled={isPending === "code"} className="bg-blue-600 text-white px-4 py-2 rounded text-sm">
            1 Year
          </button>
        </div>
          ...
          <tbody>
            {subscribers.map((clinic: any) => (
              <tr key={clinic.id} className="hover:bg-gray-50 border-b">
                ...
                <td className="p-3">{clinic.users[0]?.email || "N/A"}</td>
                <td className="p-3">
                ...
                <td className="p-3 text-sm">{clinic.subscription?.endDate ? new Date(clinic.subscription.endDate).toLocaleDateString() : "N/A"}</td>
